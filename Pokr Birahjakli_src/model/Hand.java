...
public enum Hand {
	...
	static Integer blabla ;
	static ArrayList<Integer> HighCardArray = new ArrayList<>();
	
    
	private static boolean isOnePair(ArrayList<Card> cards) {
		boolean found = false;// total value of one pair 
		
		for (int i = 0; i < cards.size() - 1 && !found; i++) {
			for (int j = i + 1; j < cards.size() && !found; j++) {
				if (cards.get(i).getCardValue() == cards.get(j).getCardValue()) {
					found = true;
					//blabla =cards.get(j).getCardValue();
				//	HighCardArray.add(blabla);
			...
			for (int j = i + 1; j < cards.size() - 1; j++) {
				for (int k = j + 1; k < cards.size(); k++) {
					if (cards.get(i).getCardValue() == cards.get(j).getCardValue()
						...
						blabla =cards.get(k).getCardValue();
						HighCardArray.add(blabla);
					}
				}
			}
